...
public class InsuranceLogic2120 {
	
	private static InsuranceLogic2120 INSTANSE;
	private MonthYearType thangBaoGiam;
	private BieuMau3A data;
	private InsuranceDAO dao;
	
	public static synchronized InsuranceLogic2120 getInstanse(BieuMau3A bieuMau3A, InsuranceDAO dao){
		if (INSTANSE==null)
			INSTANSE = new InsuranceLogic2120(bieuMau3A, dao);
		else{
	...
	private InsuranceLogic2120(BieuMau3A bieuMau3A, InsuranceDAO dao){
		this.data=bieuMau3A;
		this.dao=dao;
		xacDinhThangBaoGiam();
	}
	...
	private void xacDinhThangBaoGiam(){
		try {
			...
			int muaBHNV 	= ins.getThamGiaBHXHTN2120(data.getEmpsn(), thangNViec);
