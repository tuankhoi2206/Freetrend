 ...
 * N_EMP_JOB *
 */
public class EmpJob00MProgram extends MaintainSProgram {
	...
	protected void createDataContent() {
		setMasterDataContent(new EmpJob00MDataContent());
	}
	...
	protected int doInit() {
		...
		// btnInfo = new Button();
		// btnInfo.setIcon(new
		// ResourceImageReference("/dsc/echo2app/resource/image/info.gif"));
		// btnInfo.setDisabledIcon(new
		// ResourceImageReference("/dsc/echo2app/resource/image/infoD.gif"));
	...
	protected String[] getBrowserDisplayColumns() {
		// 4.<資料瀏覽欄位>
		return new String[] { "EMPSN", "ID_JOB", "DATE_B", "NOTE" };
	}
	...
	protected boolean doBrowserContentRefresh() {
		...
		for (int i = 0; i < ds.getRecordCount(); i++) {
			N_N_EMP_JOB data = (N_N_EMP_JOB) ds.getDataObject(i);
	...
	protected boolean doDataContentRefresh(int recNo) {
		if (super.doDataContentRefresh(recNo)) {
			DataObjectSet ds = getMasterDataContent().getDataObjectSet();
			N_N_EMP_JOB data = (N_N_EMP_JOB) ds.getDataObject();
			data.setNOTE(Vni2Uni.convertToUnicode(data.getNOTE()));
			ds.modify(data);
			ds.rollback();
