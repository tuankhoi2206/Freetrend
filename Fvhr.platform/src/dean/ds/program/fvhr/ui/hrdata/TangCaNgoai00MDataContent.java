...
public class TangCaNgoai00MDataContent extends DataContent {
	...
	private void registPropertyEditor() {
		...
		getUIDataObjectBinder().registerCustomEditor(N_N_TCA_NGOAI_NV.class,
				"THANG", MappingPropertyUtils.getStringMonthEditor());
		getUIDataObjectBinder().registerCustomEditor(N_N_TCA_NGOAI_NV.class,
				"NAM", MappingPropertyUtils.getStringYearEditor(1, false));
	...
	private void bindUI() {
		...
		getUIDataObjectBinder().addBindMap("EMPSN", EMPSN_DscField1,
				EMPSN_CaptionLabel);
		getUIDataObjectBinder().addBindMap("NAM", NAM_SelectField1,
				NAM_CaptionLabel);
		getUIDataObjectBinder().addBindMap("THANG", THANG_SelectField2,
				THANG_CaptionLabel);
		getUIDataObjectBinder().addBindMap("TCA_NGAY", TCA_NGAY_DscField2,
				TCA_NGAY_CaptionLabel);
		getUIDataObjectBinder().addBindMap("TCA_DEM", TCA_DEM_DscField3,
				TCA_DEM_CaptionLabel);
		getUIDataObjectBinder().addBindMap("TCA_CN", TCA_CN_DscField4,
				TCA_CN_CaptionLabel);
		getUIDataObjectBinder().addBindMap("TCA_LE", TCA_LE_DscField5,
				TCA_LE_CaptionLabel);
		getUIDataObjectBinder().addBindMap("SODEM_TIENCOM",
				SODEM_TIENCOM_DscField6, SODEM_TIENCOM_CaptionLabel);
		getUIDataObjectBinder().addBindMap("NOTE", NOTE_TextArea1,
				NOTE_CaptionLabel);
		// 以上為自動產生的程式碼，請勿修改此區域內之程式碼-------------------------------------------
		// ----------------------------------------------------------------------------------
	}
	...
	protected void doNewDefaulData() {
		...
		String month = c.get(Calendar.MONTH) + 1 < 10 ? "0"
