...
public class OBJ_UTILITY {
	...
	public String AutoGenKey() {
		...
		IGenericDAO<N_N_JOB, String> obj_dao = Application.getApp().getDao(
				N_N_JOB.class);
		...
		if (list != null) {
			...
			for (N_N_JOB obj : list) {
				...
				get_num = Integer.valueOf(obj.getID_JOB().replace(key, "")
						.trim());
				if (get_num > num) {
					num = get_num;
				}
	...
	public int GET_NUM_DAY_OF_MONTH_NOT_SUN(Date date_input_) {
		SimpleDateFormat sf_ = new SimpleDateFormat("dd/MM/yyyy");
		String str_date = sf_.format(date_input_);
		int month = Integer.valueOf(str_date.substring(3, 5));
		int year = Integer.valueOf(str_date.substring(6, 10));
	...
	public ArrayList<Object> sql_nfield_1row(String sql, int nfield) {
		ArrayList<Object> list = new ArrayList<Object>();
		OpenConSql();
		Statement stm = null;
		ResultSet rs = null;
	...
	protected boolean export(int recNo) {
		...
		err = executer.execute(taskIDClass, "", "", execExportType, "",
	...
	public ArrayList<Date> getAllListDate_input(Date dateFrom, Date dateTo) {
		...
		dateFrom = OBJ_UTILITY.DAY_NEXT(dateFrom);
