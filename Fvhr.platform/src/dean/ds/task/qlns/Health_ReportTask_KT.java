...
//FORM NAY DUNG CHO KTOAN XUAT HET TRONG THANG DE KIEM TRA TRU TIEN BHYT
...
import ds.program.fvhr.ngan.ui.KiemTraTienTruBHYT.KiemTraTienTruBHYTProgram;
...
public class Health_ReportTask_KT extends AListReportTask {
	...
	protected void filterHeader(HSSFWorkbook wb, HSSFSheet sheet, HSSFRow header) {
		...
		date_str = KiemTraTienTruBHYTProgram.date_str;
		date_input = null;
		try {
		...
		HANDLE_EXPORT_DETAIL(sheet0,date_input);	
	...
	private void HANDLE_EXPORT_DETAIL(HSSFSheet sheet_,Date date_input_) {
		...
		String empsn = "";
		String xuong	 = KiemTraTienTruBHYTProgram.name_fact; //28/10/2011 Ngan them de xuat chi tiet theo tung xuong
		...
		String sql1      		= KiemTraTienTruBHYTProgram.str;
	  ...
	  if(Long.valueOf(dk.toString()) == 1)
	  {
		//String sql = 
           sql =
			...
			"AND   (   ((t.status = 1 or t.status = 0)   and to_char(t.lock_date,'mm/yyyy') = '"+date_str_+"')\n" + //07/11/2011 Ngan them de chi xuat theo thang do
		...
		for (int i = 0; i < list_arr.size(); i++) {
			...
			for (int j = 0; j < arr_child.length; j++) {
				...
				case 6:     // name_dept
					...
					break;
				case 7:		// thang
					Write_Cell_for_String(sheet_,sf.format((Date)value_), i+1, j);
